...
final youngsterRef = Firestore.instance.collection('youngster');
...
class AuthService {
  ...
  Future<YoungsterStatus> get youngsterStatus async {
    await this._getYoungsterData();
    return this._user.youngster.status;
  }
  ...
  */
  ...
  Future<void> _getYoungsterData() async {
    await youngsterRef.document(this._user.id).get().then((doc) {
      if (doc.exists) {
        this._user.youngster = new Youngster.fromDocument(doc);
      }
    });
  }

  Future<void> _createYoungsterDocument(String userId) async {
    String status = YoungsterStatus.pretest_pending.toString().split(".")[1];
    await youngsterRef.document(userId).setData({"status": status});
  }

  Future<void> updateYoungsterStatus(YoungsterStatus status) async {
  ...
  Future<void> updateYoungsterData(Youngster youngster) async {
    await youngsterRef
        .document(this._user.id)
        .updateData({"altura": youngster.altura, "peso": youngster.peso, "fechaNacimiento": youngster.fechaNacimiento}).then((value) async {
      await this._getYoungsterData();
    });
  }
...
//  }
  ...
  Future<bool> registerWithEmail(String name, String email,
  ...
  Future<void> createUserDocument() async {
    ...
    if (!doc.exists) {
      ...
      await usersRef.document(currentUser.uid).setData({
        ...
        await this._createYoungsterDocument(currentUser.uid);
      });
    }
